...
- Evidence reviewed: the complete product brief in the supplied `pasted-text.txt`; the current app shell, feed, task, notification, follower, profile, settings, and theme implementation; Next.js 16 App Router guidance in `node_modules/next/dist/docs/`; WCAG 2.1 AA requirements from the brief; the approved TaskTomo dashboard theme reference captured at `.omx/artifacts/visual-ralph/tasktomo-theme/reference.png`; the 2026-08-13 direction to make the main feed structurally similar to X.com without changing the theme palette; the 2026-08-14 direction to simplify the Tasks tab into a legible “Your Tasks” workspace, remove visible XP, retain streaks, and support completion-post images; the 2026-08-14 direction to remove Followers from primary navigation and expose the AI follower directory from the profile like X; the 2026-08-14 direction to expose Settings as its own tab and make the profile’s Edit profile action a focused X-like identity editor; the 2026-08-16 direction to show feed posts immediately below feed controls without refresh-status or Community Progress interstitials; the 2026-08-16 direction to halve the visual height of post actions, remove the blue AI-post edge and action divider, and render post overflow as a plain ellipsis; the 2026-08-16 direction to make task categories optional, reusable, selectable from a dropdown, and user-manageable; the 2026-08-17 reference for an inline reply composer with the signed-in person’s avatar on the left, “Post your reply” field copy, and a visible Reply action on the right; the 2026-08-18 direction to make completion posting feel like commenting on and resharing a referenced post on X; X's official Quote-post guidance describing an optional comment/media above the referenced post; current official product and pricing material for Finch, Habitica, Focusmate, Todoist, and Sunsama; Habitica's public-community safety rationale; current Supabase Auth recovery guidance; and X's live profile hierarchy plus official profile-customization and profile-photo guidance reviewed on 2026-08-14.
...
- Organize only when useful: categories are reusable user-owned labels, never a prerequisite for creating or completing a task.
...
- New/changed components: `GoalMeter`, `PrivacyBadge`, `AITag`, `TaskRow`, `TaskCategoryManager`, `CompletionSharePrompt`, `PostMediaGrid`, compact timeline `PostCard`, `RelativeTime`, `LikeButton`, owner post controls, AI companion cards, X-inspired social profile header/tabs, focused profile editor, profile-photo upload control, password-recovery forms, settings panels, `AppTabLayout`, and `MomentumRail`. In Edit profile, the displayed circular avatar is itself the full upload trigger, with a persistent centered camera overlay; the action is not separated into a secondary upload button. Existing illustration and initials choices remain available below it. Your Tasks and Settings opt out of `MomentumRail`; Your Tasks integrates its non-duplicative streak/daily-progress context into the page header, while Settings uses the freed width for its section navigation and controls. Native task category selects always include “No category” and only reusable user-owned options. Their adjacent category manager creates and renames library entries, and requires explicit confirmation before deleting one; rename propagates to current tasks, deletion clears the category from current tasks, and already-published posts retain their historical category snapshot. Other primary tabs may retain the rail, which shows today’s tasks, daily-goal completion, and current streak without XP. On Feed, task rows and the rail footer use explicit “Your Tasks” destination language so first-time users understand where planning and completion happen. The feed uses “For you,” interest-based “Your interests,” and “People only,” an adjacent category selector, completion-gated sharing, privacy-scoped signed post media, owner-only audience/delete actions, badge-only AI identity, a divider-free compact action strip, and a plain ellipsis overflow trigger; its post stream starts directly below the controls and keeps nonvisual refresh/mutation announcements in a polite live region. Opening Reply reveals an inline composer with the signed-in person’s avatar at left, a borderless “Post your reply” field, and a visible rounded Reply button at right; the action remains disabled until the reply contains non-whitespace text. Completion posting uses one focused, single-column composer: the person's avatar and optional comment lead, selected media follows, and the immutable completed-task record appears as a bordered quoted card below. A compact audience control and media tool stay inside the composer; the primary Post action lives in its header, so the separate form-and-preview columns are not duplicated. Notifications uses “All” and “Unread” views. Social profiles use a compact cover/identity hierarchy (84px on mobile, 108px from the small breakpoint), display name plus handle, short bio, privacy state, Posts/Progress tabs, an X-style active AI companion count that links to the shared directory, and compact timeline rows without introducing human account-follow mechanics. Settings is a separate navigation destination and excludes public identity editing.
...
- Keyboard/focus behavior: 44px minimum interactive hit areas, visible 3px focus rings with surface offset, skip link, semantic landmarks, complete keyboard navigation, and no hover-only information. Category selection remains a native keyboard-operable select; category management is a labeled modal with explicit save, cancel, and delete-confirmation controls. Dense post actions and plain post-overflow glyphs may use a 36px visual box only when their hit area still extends to 44px. The full profile avatar upload target is a keyboard-operable button with an accessible name; its camera affordance remains visible without hover.
...
- Success: use a brief completion seal, clear text, streak context, `Post a win`, `Undo`, and a live-region announcement. Category create, rename, and delete also announce their exact effect.
