-- Keep direct RPC callers aligned with the 300-character completion-comment
-- contract enforced by the web and mobile clients and the API schema.
...
begin
  ...
  if p_message is not null and char_length(trim(p_message)) > 300 then raise exception 'message too long'; end if;
...
comment on function public.publish_task_completion(uuid,text,public.post_visibility,text) is
  'Publishes a completed task to the explicitly confirmed audience, preserving an omitted optional comment as blank and limiting comments to 300 characters.';
