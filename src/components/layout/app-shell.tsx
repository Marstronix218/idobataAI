...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-canvas">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[232px] border-r border-line bg-surface/90 px-5 py-6 backdrop-blur lg:flex lg:flex-col">
        <Logo />
        ...
        <div className="mt-6 rounded-2xl bg-ink p-4 text-white">
          <div className="flex items-center justify-between text-xs font-bold text-white/65"><span>Today’s goal</span><Flame size={15} /></div>
          <p className="display mt-2 text-2xl font-bold">{isPreviewMode ? 2 : profile?.daily_goal ?? "—"} <span className="text-sm text-white/60">{isPreviewMode ? "of 3 wins" : "wins planned"}</span></p>
          {isPreviewMode && <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/15"><div className="h-full w-2/3 rounded-full bg-sun" /></div>}
        </div>
