...
import { PostCard, previewFeed, type ReplyAuthor } from "@/components/social/feed";
...
export function PostThread({ postId }: { postId: string }) {
  ...
  const [replyAuthor, setReplyAuthor] = useState<ReplyAuthor | null>(isPreviewMode ? previewReplyAuthor : null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiRequest<UserProfile>("/api/profile", { signal: controller.signal }).catch(() => null),
    ])
