...
export function AuthForm({ mode, defaultEmail = "" }: { mode: "login" | "signup"; defaultEmail?: string }) {
  ...
  return (
    <form onSubmit={submit} className="mt-8 space-y-5">
      ...
      {existingEmail && <div role="alert" className="rounded-2xl border border-warning/30 bg-warning-soft p-4 text-sm leading-6 text-warning-ink"><strong>You already have an account.</strong> <span className="font-semibold">{existingEmail}</span> is already signed up, so there is nothing to create here — log in instead.<span className="mt-3 flex flex-wrap items-center gap-4"><Link className="btn btn-primary" href={`/login?email=${encodeURIComponent(existingEmail)}`}>Log in instead<ArrowRight size={17} /></Link><Link className="font-bold underline" href="/forgot-password">Forgot password?</Link><Link className="font-bold underline" href={`/resend-confirmation?email=${encodeURIComponent(existingEmail)}`}>Never confirmed it?</Link></span></div>}
      <div><label className="field-label" htmlFor="email">Email address</label><input className="field" id="email" name="email" type="email" autoComplete="email" placeholder="mina@example.com" defaultValue={defaultEmail} required /></div>
