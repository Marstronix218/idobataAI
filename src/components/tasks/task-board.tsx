...
export function TaskBoard() {
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_290px]">
    ...
    <aside className="hidden space-y-4 xl:block"><div className="goal-card rounded-[1.4rem] p-5"><div className="flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[.12em] text-muted">Today’s goal</p><Flame size={18} className="text-sun" /></div><p className="display mt-6 text-3xl font-bold">{completedToday}/{profile?.daily_goal ?? 3}</p><p className="mt-1 text-sm text-muted">Wins logged today.</p></div><div className="card p-5"><div className="flex items-center justify-between"><h2 className="display text-lg font-bold">Public progress</h2><Globe2 size={17} className="text-community" /></div><p className="mt-2 text-sm text-muted">{publicCount} tasks visible to the community. They are not feed posts.</p></div></aside>
